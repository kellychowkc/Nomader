...
const Profile = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values: UserProfile) => {
      const res: any = await updateProfile(values, userId as any as string);
      if (res.success) {
        Swal.fire({
          title: "Success",
  ...
  useEffect(() => {
    const result = getUserFriendsWithInfo(auth.id as number).then((data) => {
  ...
  useEffect(() => {
    fetchJson<Array<Post>>(`${REACT_APP_API_SERVER}/data/post`).then((data) => {
      // setAllPostList(
      //     data.map((item) => ({
      //         ...item,
      //     }))
      // )
      const result = data.filter((item) => item.username === auth.username);
      setUserPosts(result);
        ...
        <VStack w="auto">
          ...
          <Flex minH={"full"} align={"center"} justify={"center"}>
            <Stack>
              ...
              >
                <div className={styles.profileContainer}>
                  {imageStore === "" ? (
                    <Avatar
                      ...
                      size="2xl"
                      src={profileList?.profile as any as string}
                    ></Avatar>
                  ) : (
                    <Avatar size="2xl" src={imageStore}></Avatar>
                  )}
                </div>
                ...
                >
                  ...
                  <Button
                    ...
                    boxShadow={"0px 1px 2px #BBBBBB"}
                  >
                    <Link to="/editInterest">Edit Interest</Link>
                  </Button>
                </Stack>
              </Stack>
              ...
              <Stack my="3px" p="3px" spacing={4} direction={["column", "row"]}>
                <Modal
                  id="modal_Profile"
                  isOpen={isOpen}
                  onClose={onClose}
