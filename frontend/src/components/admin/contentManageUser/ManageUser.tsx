...
export default function ManageUser() {
  ...
  const reduxUserListData = useSelector((state: RootState) => state.manageUser);
    ...
    if (viewUser) {
      const result = getUserProfile(viewUser).then((data) => {
        ...
        if (data.success) {
          ...
          const result = updateUserPermission(viewUser, permissionUpdates).then(
            (data) => {
              if (data.success) {
  ...
  useEffect(() => {
    if (searchUser !== searchRef.current) {
      ...
      const result = reduxUserListData.userList!.filter(
        (item) =>
          item.first_name.match(searchUser) || item.last_name.match(searchUser)
      );
  ...
  useEffect(() => {
    if (viewFriendsById) {
      const result = getUserFriendsWithInfo(viewFriendsById as number).then(
        (data) => {
            ...
            >
              ...
              >
                ...
                >
                  ...
                  >
                    <Stack my="3px" p="3px" spacing={3} direction={["row"]}>
                      <Button
                      ...
                      <Button
                        ...
                        onClick={() => {
                          setViewFriendsById(userProfile.id);
                ...
                <Flex className="leftBoardRight">
                  <VStack>
                    <Box className="profileHistory" fontSize="sm">
                      <Text fontSize={"sm"} fontWeight={"semibold"}>
                        Member Since:
                        {" " + userProfile.created_at!.split("T", 1)}
                      </Text>
                      <Text fontSize={"sm"} fontWeight={"semibold"}>
                        Last Update:
                        {" " + userProfile.updated_at!.split("T", 1)}
                      </Text>
                    </Box>
                    <HStack className="profilePictures" spacing={0}>
                      <Box bg="teal" w="66%">
                        <Image
                          src={
                            userProfile.profile
                    ...
                    >
                      ...
                      <PermissionSetting
                        ...
                        changePermission={(name: string) => {
                          handleChange_permission(name);
                        }}
                      />
                    </VStack>
                  </VStack>
                  ...
                  <HStack>
                    <FormControl id="searchUser">
                      <Input
                        ...
                        type="text"
                        onChange={handleChange_searchUser}
                        value={searchUser}
                      />
                    </FormControl>
