...
def main():
    ...
    df_users = df_users.withColumn('birthday_year', F.split(
        df_users['birthday'], '-').getItem(2))
    df_users = df_users.withColumn('birthday_month', F.split(
        df_users['birthday'], '-').getItem(1))
    df_users = df_users.withColumn('birthday_day', F.split(
        df_users['birthday'], '-').getItem(0))
    df_users = df_users.withColumn(
        'birthday_year', df_users['birthday_year'].cast('integer'))
    df_users = df_users.withColumn(
        'birthday_month', df_users['birthday_month'].cast('integer'))
    df_users = df_users.withColumn(
        'birthday_day', df_users['birthday_day'].cast('integer'))
    df_users = df_users.drop('birthday')
    df_users = df_users.drop('updated_at')
    df_users = df_users.withColumn(
        'created_year', F.year(df_users['created_at']))
    df_users = df_users.withColumn(
        'created_month', F.month(df_users['created_at']))
    df_users = df_users.withColumn(
        'created_day', F.dayofmonth(df_users['created_at']))
    ...
    df_chat_rooms = df_chat_rooms.withColumn(
        'created_year', F.year(df_chat_rooms['created_at']))
    df_chat_rooms = df_chat_rooms.withColumn(
        'created_month', F.month(df_chat_rooms['created_at']))
    df_chat_rooms = df_chat_rooms.withColumn(
        'created_day', F.dayofmonth(df_chat_rooms['created_at']))
    ...
    df_chats = df_chats.withColumn(
        'created_year', F.year(df_chats['created_at']))
    df_chats = df_chats.withColumn(
        'created_month', F.month(df_chats['created_at']))
    df_chats = df_chats.withColumn(
        'created_day', F.dayofmonth(df_chats['created_at']))
    ...
    df_users_relationship = df_users_relationship.withColumn(
        'created_year', F.year(df_users_relationship['created_at']))
    df_users_relationship = df_users_relationship.withColumn(
        'created_month', F.month(df_users_relationship['created_at']))
    df_users_relationship = df_users_relationship.withColumn(
        'created_day', F.dayofmonth(df_users_relationship['created_at']))
    ...
    df_like_attractions = df_like_attractions.withColumn(
        'created_year', F.year(df_like_attractions['created_at']))
    df_like_attractions = df_like_attractions.withColumn(
        'created_month', F.month(df_like_attractions['created_at']))
    df_like_attractions = df_like_attractions.withColumn(
        'created_day', F.dayofmonth(df_like_attractions['created_at']))
    ...
    df_posts = df_posts.withColumn(
        'created_year', F.year(df_posts['created_at']))
    df_posts = df_posts.withColumn(
        'created_month', F.month(df_posts['created_at']))
    df_posts = df_posts.withColumn(
        'created_day', F.dayofmonth(df_posts['created_at']))
    ...
    df_browse_posts = df_browse_posts.withColumn(
        'created_year', F.year(df_browse_posts['created_at']))
    df_browse_posts = df_browse_posts.withColumn(
        'created_month', F.month(df_browse_posts['created_at']))
    df_browse_posts = df_browse_posts.withColumn(
        'created_day', F.dayofmonth(df_browse_posts['created_at']))
