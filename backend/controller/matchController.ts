...
export class MatchController {
  ...
  matchUser = async (req: Request, res: Response) => {
    try {
      ...
      const waitingUserId = await this.matchService.getUserIdWhoWaitMatchYou(
        userId
      );
      ...
      switch (allUserId.length) {
        ...
        default:
          while (
            ...
            randomId[0] =
              allUserId[Math.floor(Math.random() * allUserId.length)]["id"];
            randomId[1] =
              allUserId[Math.floor(Math.random() * allUserId.length)]["id"];
            randomId[2] =
              allUserId[Math.floor(Math.random() * allUserId.length)]["id"];
      ...
      if (randomWaitingId[0] == 0 && randomId.length > 4) {
        do {
          ...
          randomId.filter((num) => num === randomWaitingId[0]).length > 0 ||
          randomId.filter((num) => num === randomWaitingId[1]).length > 0
        );
