    ...
    matchUser = async (req: Request, res: Response) => {
        try {
            ...
            for (let wantUser of wantMatchUserId) {
                allUserId = allUserId.filter((id) => {return id["id"] !== wantUser["user2_id"] });
            }
            ...
            if (randomWaitingId[0] == 0 && randomId.length > 4) {
                do {
                    randomWaitingId[0] = allUserId[Math.floor(Math.random() * allUserId.length)]["id"];
                    randomWaitingId[1] = allUserId[Math.floor(Math.random() * allUserId.length)]["id"];
            ...
            for (let id of matchIdArr) {
                ...
                if (userData) {
                    const interestList: Array<{ title: string }> = userData["interestIdList"];
                    let interestArr = [];
