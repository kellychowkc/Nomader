...
export async function seed(knex: Knex): Promise<void> {
    ...
    const interestId: Array<{ id: number }> = await knex("interests").select(
        "id"
    );
    const postId: Array<{ id: number }> = await knex("posts").select("id");
    const citiesId: Array<{ id: number; name: string }> = await knex(
        "cities"
    ).select("id", "name");
    ...
    for (let user of userId) {
        ...
        await knex("users")
            ...
            .update({
                country_id:
                    countryId[
                        chance.integer({ min: 0, max: countryId.length - 1 })
                    ]["id"],
    ...
    for (let post of postId) {
        ...
        await knex("posts")
            ...
            .update({
                attraction_id:
                    attractionId[
                        chance.integer({ min: 0, max: attractionId.length - 1 })
                    ]["id"],
                city_id: city["id"],
                created_at: new Date(randomCreatedTime * 1000),
                updated_at: new Date(randomCreatedTime * 1000),
    ...
    for (let attraction of attractionId) {
        ...
        for (let i = 0; i < randomInterestNum; i++) {
            let interest: number;
            do {
                interest = chance.integer({
                    min: 0,
                    max: interestId.length - 1,
                });
    ...
    const chatRoomId: Array<{ id: number }> = await knex("chat_rooms").select(
        "id"
    );
    ...
    const browsePostId: Array<{ id: number }> = await knex(
        "users_browse_posts"
    ).select("id");
    ...
    const browseAttractionId: Array<{ id: number }> = await knex(
