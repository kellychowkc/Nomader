...
export async function up(knex: Knex): Promise<void> {
  ...
  const hasDBEmergencyData = await knex.schema.hasTable("db_emergency_data");
  ...
  const hasDBAttractions = await knex.schema.hasTable("db_attractions");
  if (!hasDBAttractions) {
  ...
  const hasDBCurrencyCodes = await knex.schema.hasTable("db_currency_codes");
  ...
  const hasDBCurrencyRates = await knex.schema.hasTable("db_currency_rates");
