...
export class GetDataService {
  ...
  async getHotPostData() {
    const hotPostList = await this.knex
      ...
      .from("posts")
      .innerJoin("users", "posts.user_id", "users.id")
      .innerJoin("users_browse_posts", "posts.id", "users_browse_posts.post_id")
      .groupBy(
  ...
  async getCurrencyRates(code: string) {
    ...
    const codeId: { id: number } = await this.knex("currency_codes")
      .select("id")
      .where("code", code)
      .first(); // null or object
    const currencyData: Array<{
      ...
      .andWhere("currency_rates.code_base_id", codeId["id"]);
    return currencyData;
  }
