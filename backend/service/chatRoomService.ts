...
export class ChatRoomService {
    ...
    async getAllChatInfo(userId: number) {
        const roomAsManager: Array<{ id: number }> = await this.knex(
            "chat_rooms"
        )
            ...
            .select(
                ...
                "users.profile",
                "chat_rooms.id",
                "chat_rooms.room_title",
                "chat_rooms.user_manager_id",
                "chat_rooms.user_member_id",
                "chat_rooms.updated_at"
            )
            .where("chat_rooms.user_member_id", userId);
        const roomAsMember: Array<{ id: number }> = await this.knex(
            "chat_rooms"
        )
